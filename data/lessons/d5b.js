...
'- **FBX** — the long-standing game-industry workhorse. Proprietary (Autodesk) but supported everywhere, and the safest default for skinned meshes, rigs and animation into Unity/Unreal.\n' +
'- **glTF / GLB** — an open, modern standard. `.glb` is a single self-contained binary (mesh + textures + materials in one file), which is tidy and great for the web and Godot. Engine support keeps improving; for a rigged character into Unity, FBX is still the lower-risk choice today.\n\n' +
...
'**Axis convention** differs too — Blender is **Z-up**, most engines are **Y-up** — so the exporter has a forward/up setting to convert. Get it wrong and assets lie on their side. And the **origin** you set in [[d5-02]] is the pivot the engine snaps and rotates around, so it must be deliberate.\n\n' +
'Clean export is teamwork, not pedantry: a mis-scaled asset becomes the **level designer’s** ([[c3-02]]) broken snapping and the **programmer’s** ([[e2-01]]) wrong collider. "Imports clean at correct scale" is part of *done*.',
    steps: [
      ...
      'Export FBX: **File → Export → FBX**. Set **Limit to → Selected Objects**, **Apply Scale → "FBX All"** (a common fix when scale still arrives wrong), **Forward `-Z`, Up `Y`** for a Y-up engine, and bake/keep only the modifiers you want.',
      'Or export glTF: **File → Export → glTF 2.0**, choose **`.glb`** (self-contained binary) and enable the data you need (mesh, materials, and — only if rigged — skinning + animation).',
      'Import into the engine and immediately check three things: real-world size against a 1 m reference cube, upright orientation, and pivot location. If size is off by ~100×, it’s almost always a metre-vs-centimetre unit mismatch — fix at export, not by scaling in-engine.'
    ],
    task:
'Take one finished prop (or a free asset you can open). Before touching the exporter, write down its intended real-world size in metres and where its pivot should be. Then run the export checklist above twice — once to **FBX**, once to **glTF/GLB** — and note, in a short paragraph, any difference you’d expect on import (textures embedded or not, animation support, file tidiness) and which you’d choose for *this* asset and why.',
    success: [
    ...
    simplified: 'Exact exporter labels and defaults shift between Blender versions and engines (and Unity’s old FBX cm/m quirk has changed over time). Treat the specific menu names as common-default guidance — verify against your Blender and engine version, and always sanity-check scale against a known 1 m reference in-engine.',
    goDeeper: 'Read the official Blender FBX/glTF exporter docs alongside your engine’s mesh-import docs (Unity "Importing models", Unreal "FBX Import"). The glTF spec site (Khronos) is the authority on what GLB actually packs.',
    quiz: [
      ...
      { q: 'When would you pick `.glb` over FBX, and when the reverse?', a: 'Pick `.glb` when you want one tidy self-contained file (mesh, materials, textures together) — great for the web, Godot, and simple props. Prefer FBX today for rigged/animated characters into Unity or Unreal, where it remains the most battle-tested, lowest-risk path.' }
    ],
...
'Draft a one-page **naming convention** for a small game’s 3D assets: choose your prefixes (meshes, skeletal meshes, materials, textures), your separator and case rule, your number-padding rule, and your texture-map suffixes (tie them to the PBR channels). Then rename three real files you have (or three invented ones like `Cube.001`, `rock final`, `tex2`) to follow it, and write one sentence on what each old name would have cost a teammate.',
    ...
    simplified: 'The specific prefixes (`SM_`, `T_`, …) are conventions, not laws — Unreal popularised one style; Unity/Godot teams pick their own. What matters is that you choose a clear scheme and apply it consistently across the whole project, not which exact letters you use.',
    goDeeper: 'The widely shared "Unreal Engine style guide" community document is a thorough, opinionated reference for asset naming and folder structure; adapt its ideas to your engine and team rather than copying it wholesale.',
    quiz: [
...
'For one textured asset, write its **import recipe**: target texture max-size and why, which maps are sRGB vs linear (list each PBR map and tag it), mipmaps on/off, whether it needs LODs and collision, and whether rig/animation import should be off. Then name *one* thing you would NOT optimise yet and explain how you’d use a profiler to decide whether it’s ever worth it.',
    ...
    goDeeper: 'Your engine’s texture- and mesh-import documentation plus its profiler docs (Unity Profiler / Memory Profiler, Unreal Insights / "stat" commands) are the real teachers; the skill is reading a frame to find the actual bottleneck before you change anything.',
    quiz: [
