...
'- **Match the engine’s skeleton when you can.** In Godot you import a rigged character (prefer **glTF/`.glb`**) and it becomes a `Skeleton3D`; a clean, conventionally-named humanoid skeleton lets you *retarget* shared animation between characters — a huge time-saver for a small team.\n\n' +
...
'Take a character or creature you could model (or one you have) and **plan its skeleton on paper** before touching Blender. Draw the bone hierarchy as a tree: root at the hip, spine chain, head, two arm chains, two leg chains, and any extras the design actually needs (a tail? a jaw? fingers, or just a single hand bone?). Beside it, write a rough **bone budget** and one sentence on *why* — is this a lone hero you see up close, or a crowd enemy where every bone multiplies? Cutting finger bones you’ll never animate is scope discipline, not laziness.',
    ...
    goDeeper: 'The official Blender rigging/Armature docs and Godot’s docs on importing 3D scenes, `Skeleton3D` and retargeting are the real next step; search reputable "game character rigging" breakdowns for full worked examples.',
    quiz: [
...
'- **Apply transforms, sane bind pose**, and decide whether you export the mesh and skeleton once and animations separately, or all together — small teams usually split *skeleton+mesh* from *clips* so animations are reusable. In Godot, importing a **glTF/`.glb`** brings the clips in on an `AnimationPlayer` node, and you can re-save imported animations as separate `.res`/`.tres` library files to share them.\n\n' +
...
'Write a one-page **rig handoff sheet** for a character — the doc you’d hand the animator and programmer. Include: the bone-naming convention (with the exact left/right suffix you’ll use), the root-bone decision (**root motion** or **in-place**, and one line on why), export scale/units and forward axis, and how you’ll split the export (mesh+skeleton vs animation clips). Then list two things you’ll *strip or rename* before export. This sheet is the seam: it’s what makes your rig usable by people who aren’t you.',
    ...
    simplified: 'Whether to use root motion, and how to split exports, is a team/engine choice with real trade-offs — root motion is great for grounded movement but harder to blend and network; in-place is simpler to control. Exact bone-name standards differ per source (Godot’s humanoid retarget profile, Mixamo, and various Blender rig conventions); pick one and be consistent rather than assuming a universal standard.',
    goDeeper: 'Read Godot’s docs on importing 3D scenes, the `AnimationPlayer`/`AnimationTree`, retargeting (the humanoid bone-map profile) and root motion (`RootMotionView` and `get_root_motion_position`), and look at an established naming convention (e.g. a standard humanoid skeleton) to see why consistency enables retargeting.',
    quiz: [
