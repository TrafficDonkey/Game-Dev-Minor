...
'In engines you rarely write A* yourself: in **Godot** you bake a navmesh into a `NavigationRegion2D/3D`, drop a `NavigationAgent2D/3D` on the mover, set `agent.target_position`, and each frame call `agent.get_next_path_position()` and steer toward it (the `NavigationServer` does the A*). Same idea elsewhere — Unity bakes a NavMesh and you call `NavMeshAgent.SetDestination`; Unreal has the Navigation System and `AIMoveTo`. It is always graph plus A*, wrapped in a node/component. Knowing what is underneath is what lets you debug "why won’t this agent go through the door?" (usually a hole in the bake).',
    task:
'Open the **Code playground** and run the grid A* demo: place a start, a goal and some walls, and watch which cells it expands. Then on paper, take a 5×5 grid with two wall cells, mark start and goal, and hand-trace the first **three** node expansions — writing `g`, `h` (use Manhattan distance) and `f` for each. Finally, in two sentences, explain why a navmesh needs far fewer nodes than a 1-metre grid covering the same room, and name the engine nodes you would actually use in Godot (`NavigationRegion2D/3D` to bake + `NavigationAgent2D/3D` to follow; the equivalents are Unity’s `NavMeshAgent` and Unreal’s `AIMoveTo`).',
    success: [
      ...
      'You can name the engine node that bakes/runs navigation (Godot’s `NavigationRegion` + `NavigationAgent`) and a common failure (a hole in the bake).'
    ],
    ...
    goDeeper: 'Amit Patel’s "Red Blob Games" interactive A* guide is the clearest visual explanation anywhere; pair it with the Godot Navigation docs (`NavigationServer`, `NavigationRegion`, `NavigationAgent`).',
    quiz: [
