...
'- **Blender → Godot.** You model and texture game-ready ([[d5-06]]), then **export** a clean `.glb` (Godot has first-class glTF import; `.fbx` works but glTF is the cleaner path) with correct scale and units ([[d5-03]]) into the project’s asset folder. The seam is the export settings *and* a consistent naming convention so the file is recognisable months later.\n' +
'- **Godot.** You import the asset, build a `Scene` (a saved tree of `Node`s, `.tscn`) or blockout ([[c4-01]]), and script a mechanic in GDScript — or C#, both are fully supported ([[e2-01]]). The engine is the *integration point* where all five tracks finally meet.\n' +
'- **Everything → Git.** Code, scenes, and small assets are versioned and pushed ([[03-01]]). Large binaries go through **Git LFS**, and a good `.gitignore` keeps Godot’s generated editor cache (the `.godot/` folder) out of the repo ([[03-04]]). A real bonus: Godot’s `.tscn`/`.tres` scenes and resources are **text-based, so they diff and merge in Git far better than Unity’s scene format**.\n\n' +
...
'Set up a tiny end-to-end pipeline on your own machine and **commit it**. Create a project folder with a deliberate structure, initialise Git, add a `.gitignore` for your engine, make a throwaway asset in Blender, export it into the engine project, and push the result. The asset can be a single textured cube — the point is the *path*, not the art. Write a 3-line `README` describing where each kind of file lives.',
    steps: [
      ...
      'In a terminal at the root: `git init`, then create a `.gitignore`. For Godot 4.x, ignore at least `engine-project/.godot/` (the editor cache) — do **commit** `project.godot`, your `.tscn`/`.tres` and the `.import` files (check Godot’s official `.gitignore`).',
      'Set up Git LFS for big binaries: `git lfs install`, then `git lfs track "*.glb" "*.png" "*.blend"` — this writes a `.gitattributes` file ([[03-04]]).',
      'In Blender: model a cube, give it a simple material, and **export** to `engine-project/assets/models/` as `.glb` (preferred for Godot) or `.fbx`, applying transforms and checking scale/units ([[d5-03]]).',
      'In Godot: open `engine-project`, confirm the model imports at the right size, drop it into a scene as a `Node` and save the scene (`.tscn`).',
    ...
    success: [
      ...
      'Your repo commits source, `.tscn` scenes and `project.godot` but ignores Godot’s `.godot/` cache, with LFS handling binaries.',
    ...
    simplified: 'Exact ignore lists, LFS tracking and import settings are version-specific — always start from Godot’s official `.gitignore` and docs rather than this sketch.',
    goDeeper: 'GitHub publishes a maintained `.gitignore` template for Godot; pair it with Godot’s "version control" and "import" docs and the Git LFS guide.',
    quiz: [
      { q: 'Why ignore Godot’s `.godot/` cache folder in Git?', a: 'It is regenerated by the editor from your source assets and `.import` files every time the project opens. Committing it bloats the repo, causes constant merge conflicts, and stores nothing you can’t rebuild. Version the *sources* (and Godot’s text-based `.tscn`/`.tres`); let the editor regenerate the cache.' },
...
'- **Stand up the shared toolchain.** One Godot version everyone uses (and one language convention — GDScript or C# — agreed up front), a shared Git repo with LFS and a `.gitignore` agreed *before* anyone commits ([[03-04]]), and a project structure decided together ([[f3-01]]).\n' +
'- **Start the GDD and the board.** A one-page pitch and design pillars to align everyone ([[f0-01]]), and a Trello (Kanban) board so work is visible from day one ([[02-05]]).\n\n' +
'Notice what is *not* in week one: polished art, a clever mechanic, a vertical slice. Those come after alignment. The classic trap is a team that spends week one prototyping in eight slightly different Godot versions on no shared repo, then loses week three to merge chaos and a scope nobody agreed to.\n\n' +
'Your personal week-one goal is narrower: **be the person who makes the team’s pipeline real.** Help set up the repo, the structure and the agreed Godot version; draft the first scope cut; and make one tiny thing travel end-to-end ([[f3-01]]) so the team has a *working skeleton* to build on. Setup is unglamorous and it is exactly where a versatile dev earns their place.',
    task:
'Write your own **week-one plan** as a dated checklist (Mon–Fri) for the first week of the minor, assuming an eight-person team and one shared game. For each day, list 1–3 concrete outcomes (e.g. "repo + `.gitignore` + LFS agreed and pushed", "one-page pitch drafted", "scope cut #1 — three features dropped"). Mark which items *you* would personally drive given your five-track range, and put a star next to the single item that, if skipped, would hurt the team most.',
    success: [
      ...
      'You can name the concrete week-one artifacts: shared repo + ignore/LFS, agreed Godot version, one-page pitch, a board.',
    ...
    quiz: [
      ...
      { q: 'What should a team agree about Git *before* anyone makes a commit?', a: 'The shared repo, the `.gitignore` (so Godot’s `.godot/` cache never enters history), and Git LFS for large binaries. Agreeing these up front prevents bloated history and the merge chaos that hits teams who each start in their own ad-hoc setup.' }
    ],
...
'- **Tools work, and connect.** Godot installed and opened a project; Blender exports a game-ready asset (prefer `.glb`) that imports at correct scale ([[d5-03]]); Git installed with a remote you can push to, LFS set up, a sane `.gitignore` that ignores `.godot/` ([[03-04]]); and you’ve run *one* asset all the way through the pipeline ([[f3-01]]).\n' +
