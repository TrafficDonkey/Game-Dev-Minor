...
'Plan one realistic two-week sprint for a tiny team game on paper. Write a **sprint goal** in one sentence ("a player can walk into the test room and pick up a key"). Then list **five to eight backlog items** that serve that goal, each tagged with the track it belongs to — Story, Design, Level, 3D, Code or Team — and each small enough to finish in two weeks. Finally, write the two questions your **sprint review** demo must answer, and one thing you would put on the **retrospective** agenda before the sprint even starts (a risk you already suspect).',
  ...
  {
    id: '02-05', title: 'Your Trello board: Kanban in practice', pillarId: '0', phaseId: '02', moduleId: '02b',
    difficulty: 'Intermediate', mode: 'handson', estMinutes: 25,
    diagram: 'kanban',
    concept:
'Good news about your team using **Trello**: a Trello board *is* a Kanban board. **Kanban** is the *method* — make the work visible and limit how much is in progress; **Trello** is the *tool* that implements it, where vertical **Lists** are your columns and **Cards** are your tasks. So everything below applies straight to the board your team will run. Where Scrum gives you a *cadence* ([[02-04]]), Kanban gives you a *flow*, from two ideas a small team can adopt in an afternoon.\n\n' +
'**1. Make the work visible.** Put every task on a **card** and every card on a board with **lists** for the stages work passes through — a common game-team flow is **Backlog → To Do → In Progress → Review → Done** (five Trello lists). Anyone can glance and read the whole project: what is queued, being built, waiting on a check, or shipped. For a multidisciplinary team that is gold — a card **labelled** *3D* sitting in *Review* tells the level designer their asset is nearly ready to place ((handoff)). Use Trello **labels** for the track and add card **members** so ownership is obvious.\n\n' +
'**2. Limit work-in-progress (WIP).** This is the idea most people miss, and the powerful one. Cap how many cards may sit in *In Progress* at once — say three. When it is full you may **not** start anything new; you must *finish* something and move it on. It feels wrong ("but I have spare time!") yet it is exactly right: **starting work is not the goal — finishing it is.** A board with twelve things half-done has shipped nothing. A WIP limit makes the team swarm to *Done*, surfaces bottlenecks (cards piling before *Review* means review is the constraint), and kills the student-project killer: a pile of 80%-finished features the night before the deadline. One Trello caveat — it does **not** enforce a WIP cap by default; you keep it by team agreement (or add a free Power-Up that counts cards per list). The practice board above *does* enforce it, so you can feel why it works.\n\n' +
'Kanban and Scrum are not rivals — many teams run Scrum’s cadence *and* a board for the day-to-day flow within each sprint ((Scrumban)). The board is also where your **definition of done** lives: a card reaches *Done* only when it meets an agreed bar (built, reviewed, integrated — not just "works on my machine"), which ties straight into code review and pull requests ([[03-03]]).\n\n' +
'One caution: never let colour be the *only* signal. If you colour-code tracks with Trello **labels**, also put the track’s name on the card, so a colourblind teammate reads the same board you do — a habit the **Sprint Board** tool above follows deliberately.',
    steps: [
      'Open the **Sprint Board** tool above — it mirrors a Trello board. It seeds an example across five lists: Backlog → To Do → In Progress → Review → Done.',
...
'Using the **Sprint Board** tool above (your stand-in for the team’s Trello board), model the *first sprint* of a tiny team game. Seed it with six to eight real cards spread across at least four tracks (you must include at least one Code, one 3D or Level, and one Team card). Drive the board for a simulated few days: pull work into *In Progress* only up to the WIP limit, push finished cards through *Review* to *Done*, and deliberately let one card pile up before *Review* so you can see a bottleneck form. Then write three sentences: which track was the **constraint** (where cards queued), what the **WIP limit forced** you to do differently, and one rule you would add to your team’s **definition of done** so a card can’t reach *Done* too cheaply.',
    ...
    goDeeper: 'David J. Anderson’s *Kanban* is the canonical book; for a free start, the *Kanban Guide* and any "WIP limits explained" talk will reinforce why finishing beats starting. In **Trello**, explore **labels**, card **members**, **checklists** and a card-count **Power-Up** so the board does this for real.',
    quiz: [
    ...
    tags: ['trello', 'kanban', 'wip', 'workflow', 'teamwork', 'definition of done'] }
  );
