...
;(function (root) {
  ...
  D.lessons.push(
  {
    ...
    concept:
'*This is the graphics branch — it pairs with everything you learned in 3D (Pillar D). The GPU concepts here are **universal**: the same pipeline runs under Unity, Unreal and Godot. Where a tool name appears, it is flagged as engine-specific.*\n\n' +
...
'Modern engines wrap this in a **render pipeline** you pick (Unity’s URP vs HDRP; Unreal’s deferred/forward), but the vertex → rasterize → fragment → merge spine is constant underneath.',
    task:
'Draw the pipeline as a five-box flow (vertex → rasterize → fragment → output merge, with "mesh in" before and "pixel on screen" after). For each box write one sentence on what it does. Then take one asset you made in Pillar D and trace it: how many vertices roughly enter vertex processing, and which of *your* texture maps get sampled in the fragment stage? Finally, answer in one line: if a screen-filling fog effect is slow, is that more likely a **vertex** cost or a **fragment** cost — and why?',
    ...
    goDeeper: 'The opening chapters of *Real-Time Rendering* (Akenine-Möller et al.) are the standard reference; for a gentle visual intro, the official Unity/Godot render-pipeline docs and "LearnOpenGL" walk the same stages.',
    quiz: [
...
'*Engine-specific (Unity/C#) — you usually write shaders in **HLSL** inside Unity, **GDScript-like Godot shading language** in Godot, and HLSL/material nodes in Unreal. The language differs; vertex-out-position and fragment-out-colour, uniforms vs interpolants, and the math are the same everywhere.* Most game devs reach for a **shader graph** ([[e5-03]]) before hand-writing code — but reading this lesson is why the graph’s nodes make sense.',
    task:
'On paper, label which stage each effect belongs in (vertex or fragment): (a) grass swaying in wind, (b) a glowing rim of light on a character’s edge, (c) scrolling a water texture, (d) flattening a mesh into the ground as it dies, (e) tinting everything red when the player is hurt. Then take the pseudocode above and describe, in words, what you would change to make the surface pulse brighter over time using a `time` uniform (hint: multiply the colour by something derived from `time`).',
    ...
    simplified: 'GLSL-flavoured pseudocode is shown for readability; real Unity shaders are HLSL and wrap this in more boilerplate (passes, the URP/HDRP framework). The diffuse term is the simplest lighting model — real PBR shading (from [[d3-01]]) adds specular, energy conservation and more. Concepts transfer; syntax does not.',
    goDeeper: 'For hand-written shaders, "The Book of Shaders" (Gonzalez Vivo & Lowe) teaches fragment shaders interactively; Freya Holmér’s shader talks and Unity’s/Godot’s shading-language docs are excellent for the game-engine flavour.',
    quiz: [
    ...
    tags: ['shaders', 'vertex shader', 'fragment shader', 'glsl', 'uniforms', 'engine-neutral'] },
  {
    ...
    concept:
'*Engine-specific (Unity/C#) — this lesson uses Unity’s **Shader Graph**. The same idea is the **Material Editor** node graph in Unreal and the **VisualShader** in Godot. The nodes and wires map almost one-to-one; the concept is fully transferable.*\n\n' +
'A **shader graph** is a visual, node-based way to build a shader without writing HLSL by hand. You drop **nodes** (sample a texture, add, multiply, a `Time` node, a `Fresnel` node) and wire their outputs into inputs, ending at a **master/output node** whose slots are exactly the PBR channels you learned in [[d3-01]]: Base Color, Normal, Metallic, Smoothness/Roughness, Emission. Under the hood the graph *compiles to* a real vertex+fragment shader ([[e5-02]]) — the graph is a friendlier face on the same machine.\n\n' +
...
'You cannot run an engine in this course, so the steps below are the exact click-path and node graph; build it for real in your engine and watch it preview. Keep the scope tiny: one node graph, one effect, one tweakable strength — that is a complete, shippable shader.',
    steps: [
      '1. In Unity (URP or HDRP), right-click in the *Project* window → **Create → Shader Graph → URP → Lit Shader Graph**. Name it `RimGlow`.',
      '2. Double-click it to open the Shader Graph editor. You see the **Fragment** (and Vertex) master stack on the right with PBR slots (Base Color, Metallic, Smoothness, Emission…).',
      '3. Right-click the canvas → **Create Node → Math → Vector → Fresnel Effect**. It outputs a value near 0 facing the camera and near 1 at glancing edges.',
      '4. Add a **Power** node; wire Fresnel → Power’s `In`. Expose its exponent to control how tight the rim is (higher = thinner edge).',
      '5. Create a **Color** property in the Blackboard (the panel on the left) named `_RimColor`; drag it in. Multiply (`Multiply` node) the Power output by `_RimColor`.',
      '6. Wire the Multiply result into the master stack’s **Emission** slot so the rim is self-lit. Optionally also feed Base Color from a texture sample for the body of the material.',
      '7. Add a **Float** property `_RimStrength` (default 1), multiply it into the chain, and tick *Exposed* so it shows on the material in the Inspector.',
      '8. Click **Save Asset**. Create a Material from the graph (right-click the graph → Create → Material), assign it to a mesh in the scene, and tweak `_RimColor`/`_RimStrength` live in the Inspector — [[Ctrl+S]] the scene.'
    ],
    task:
'Build the `RimGlow` graph from the steps above in your engine (Unity Shader Graph, or the equivalent Unreal Material / Godot VisualShader). Apply it to an asset you made in Pillar D. Then **extend it once**: add a `Time` node and a `Sine` so the rim **pulses**, wiring the result into the strength — exactly the animated-uniform idea from [[e5-02]]. Write three sentences: what each of Fresnel, Power and Emission contributed, and one gameplay use you would put this effect to (selection highlight, low-health warning, interactable prompt).',
    success: [
    ...
    simplified: 'Exact menu names and node names are version-dependent (URP vs HDRP, Shader Graph version, and the Unreal/Godot equivalents differ) — treat the click-path as a common-defaults guide and check your version. The Fresnel node hides some math (it is a reshaped view-normal dot product); the principle is what matters.',
    goDeeper: 'Unity’s official Shader Graph docs and sample library, Ben Cloward’s shader-graph tutorial series, and Godot’s VisualShader docs all build effects node-by-node from here (dissolve, hologram, water).',
    quiz: [
      { q: 'A shader graph avoids writing code — does that mean it is not really a shader?', a: 'No. The graph is a visual authoring tool that compiles down to a real vertex+fragment shader running on the GPU. The nodes and wires become the same kind of program you would otherwise hand-write in HLSL — you have just built it visually.' },
    ...
    tags: ['shader graph', 'fresnel', 'rim glow', 'emission', 'node-based', 'engine-specific'] },
  {
...
'*Engine-specific — Unity ships two systems: the older **Shuriken** (component-based, CPU) and **VFX Graph** (GPU, for huge counts). Unreal has **Niagara**; Godot has **GPUParticles**. The emitter → initial → over-lifetime → render model is shared by all of them.*',
    task:
'Pick one effect — a melee hit spark, a magic pickup, or a dust puff on landing — and write a one-page "particle spec" a teammate could build: emission (burst or rate, count), initial velocity/size/colour with their random ranges, what changes over lifetime (size curve, colour/alpha fade), the texture and blend mode (additive vs alpha), and the lifetime in seconds. Then add a **budget line**: the maximum particle count you would allow on screen at once, and one sentence on why over-spawning hurts performance.',
    ...
    simplified: 'This is the standard billboard-particle model. Modern GPU systems (Unity VFX Graph, Unreal Niagara) simulate millions of particles on the GPU and add meshes, ribbons/trails, collision and flipbook animation — more power, same core concepts. Exact module/menu names are version-dependent.',
    goDeeper: 'Unity’s VFX Graph and Particle System docs, Unreal’s Niagara learning path, and the GDC talks on real-time VFX (Sirlin/various) go deep on timing, shape and the art of "feel".',
    quiz: [
...
'*Engine-specific — Unity uses **post-process Volumes** (URP/HDRP Volume framework, formerly the Post-Processing Stack); Unreal uses a **Post Process Volume** with the same effect set; Godot exposes them on the `Environment`/`WorldEnvironment`. The effects and their meaning are identical across engines; only where you tick the boxes differs.*',
    task:
'Take a screenshot of any game you own and write down which post-processing effects you can identify (look for glow around lights = bloom, a colour cast = grading, soft far backgrounds = depth of field, darkened corners = vignette, grounded shadows in creases = AO). Then write a **three-effect "house grade"** you would apply to a calm, warm indoor level and a **different** three-effect grade for a cold, tense one — and one sentence on a readability risk you would watch for in each.',
    ...
    simplified: 'Effect names, the Volume/profile workflow and defaults are version- and pipeline-dependent (URP vs HDRP differ; some effects are HDRP-only). The "full-screen pass that re-shades the frame" model holds everywhere; treat specifics as common defaults to verify in your version.',
    goDeeper: 'Unity’s URP/HDRP Volume and post-processing docs, Unreal’s Post Process Volume reference, and colour-grading talks/LUT tutorials go deep; for the why, any DOP/cinematography primer on grading and mood transfers directly.',
    quiz: [
