...
'The metallic/roughness workflow (Godot’s `StandardMaterial3D` defaults to it, as do the other major engines) uses these maps:\n' +
...
'Take any real object near you — a mug, a worn tool, a phone — and *channel-map* it in writing. For each PBR map, describe what its texture would contain: albedo (the flat colours, no shine), metallic (which parts are metal, as a black/white mask), roughness (which parts are glossy vs matte, light-to-dark), normal (what fine bumps/scratches you’d bake), and AO (where crevices darken). Then name one place you might be *tempted* to paint a highlight or shadow into albedo, and say why that belongs in roughness or the lighting instead.',
    ...
    goDeeper: 'The original Allegorithmic/Adobe "PBR Guide" (free, two volumes) is the standard reference on the theory and on authoring valid albedo and metallic values; pair it with the Godot docs on `StandardMaterial3D` (and the spatial shader reference) to see how the engine consumes these channels.',
    quiz: [
...
'- **Ground-truth vs approximated lighting.** Offline renderers can path-trace true global illumination, soft shadows and accurate reflections by shooting many rays. Real-time *approximates*: in Godot 4.x that means baked lightmaps (`LightmapGI`), reflection probes, screen-space reflections, and real-time global-illumination approximations like `SDFGI` and `VoxelGI` that a capable GPU like your RTX 3060 Ti can afford — but always within budget, with trade-offs and fallbacks.\n' +
...
'Take one material you’ve built (or seen) as a **procedural node graph** in Blender — something using noise, gradients or math nodes. Write a short "bake-down plan": which final maps you’d bake it to (albedo, roughness, normal, AO, maybe a packed ORM), at what resolution, and what *would be lost* by flattening it (e.g. infinite detail on zoom, view-dependent tricks). Then state one thing that material does which a real-time shader genuinely *can* do live, and one thing it can’t afford.',
    ...
    goDeeper: 'Read Godot’s rendering and global-illumination docs (the Forward+/Mobile renderers, plus `LightmapGI`, `SDFGI` and `VoxelGI`) on how it approximates global illumination, then compare a Blender Cycles render of an asset to the same asset in-engine to feel the difference in cost and look.',
    quiz: [
