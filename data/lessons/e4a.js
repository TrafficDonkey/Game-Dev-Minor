...
;(function (root) {
  ...
  D.lessons.push(
  {
    ...
    concept:
'*This branch is **engine-neutral** (Layer 1): the structures here — state machines, behaviour trees, perception — are true in every engine. Where it helps, it points at the Godot 4.x counterpart (in **GDScript or C#**); the same idea holds elsewhere.*\n\n' +
...
'Each tick it tries to attack; if it can’t, it tries to hunt the last-known position; if it can’t, it patrols. Want the guard to **flee at low health**? Add one `Sequence: [ LowHealth? -> FleeToCover ]` near the top of the Selector — *one* insertion, no rewiring. That is the readability and scalability designers love. **Godot 4.x** has no built-in BT editor, but you can hand-roll one from a few `Node` scripts (each node returning Success/Failure/Running) or add an Asset Library plugin like **LimboAI** or **Beehave** for BTs as **visual graphs** a designer can edit without code. (Same idea elsewhere: Unreal ships a BT editor.)\n\n' +
