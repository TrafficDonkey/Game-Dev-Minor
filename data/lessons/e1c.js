...
'This is sometimes called publish/subscribe or an event bus, and it is the backbone of decoupled game code. The payoff: you can add a new reaction (an achievement, a tutorial hint) by *subscribing*, with **zero changes** to the coin. In **Godot** this pattern is built into the engine as ((signals)): you declare `signal coin_collected(value)`, fire it with `coin_collected.emit(value)`, and a listener connects with `coin.coin_collected.connect(_on_coin_collected)`. It is the same idea as the DOM `addEventListener` you may know from JS (same idea elsewhere: C# `event`/`delegate`, Unity’s `UnityEvent`, Unreal’s delegates) — same pattern, different names, fully transferable.\n\n' +
...
'Take the coin example and **add a fourth reaction** without touching the coin: a `quest.recordCoin()` subscriber. Write the one line that registers it. Then write two sentences: (1) what would have to change in the *tightly-coupled* version to add that same reaction, and (2) one concrete risk of the event version (name the unsubscribe/leak problem or the "hard to trace flow" problem). Finally, list three real game moments that are natural events to broadcast (e.g. `PlayerDied`, `LevelLoaded`, `HealthChanged`).',
    ...
    simplified: 'Real event systems add typed payloads, ordering/priority, sync-vs-deferred dispatch and weak references. The publish/subscribe core here is what transfers; engine specifics (Godot signals, and the same idea elsewhere as C# events) layer on top.',
    goDeeper: 'See the "Observer" chapter of *Game Programming Patterns* (Nystrom), and the Godot docs on signals (the engine’s built-in observer mechanism) — in C# the equivalent is the standard `event`/`delegate`.',
    quiz: [
    ...
    tags: ['events', 'observer', 'pub/sub', 'decoupling', 'signals', 'engine-neutral'] },
  {
    ...
    concept:
'*Layer-1, engine-neutral: this is the mental model the whole engine layer is built on — meet it now and Godot’s scene tree of nodes ([[e2-02]]) will feel obvious later.*\n\n' +
'New programmers reach for deep **inheritance** to model a game world: `Entity` → `Character` → `Enemy` → `FlyingEnemy` → `FlyingShootingEnemy`. It collapses fast. What about an enemy that flies *and* explodes? Or a barrel that takes damage like an enemy but never moves? A single tree of "is-a" relationships cannot express the messy reality that game objects are really *bundles of capabilities*. This is the classic ((inheritance vs composition)) problem.\n\n' +
...
'This is exactly how mainstream engines model the world. **Godot** composes a tree of **Nodes**: each node is a focused capability (`Sprite2D`, `CollisionShape2D`, `CharacterBody2D`, an `Area2D` for triggers, your own `Health.gd` script on a node), and you build a thing by assembling child nodes into a **scene** ([[e2-02]]) rather than carving a class hierarchy. (Same idea elsewhere: Unity attaches components to a GameObject; Unreal uses Actors with Components.) The habit transfers wholesale.\n\n' +
...
'Pick three game objects from a game you know — say a **player**, a **patrolling enemy**, and a **pickup/door**. For each, write its component list (use names like `Transform`, `Health`, `Collider`, `Renderer`, `AIController`, `Interactable`). Then show the power of composition: describe **one new object** you can build *purely by remixing* those components (e.g. a `Transform + Health + Collider` destructible crate, or a `Transform + Interactable` save-point with no health). Finally, write one sentence on why an inheritance tree would have struggled with that same object.',
    success: [
      ...
      'You can build a new object by remixing components, and connect it to Godot’s nodes and scenes.'
    ],
    skills: ['Composition over inheritance', 'Entity-component thinking', 'Mapping to engine objects'],
    simplified: 'Engine composition (Godot’s nodes-in-a-scene-tree) and strict data-oriented ECS are related but not identical; ECS’s array-of-components + systems design is a performance specialisation. The transferable core is composition over inheritance.',
    goDeeper: 'The "Component" chapter of *Game Programming Patterns* (Nystrom) is the clean introduction; for the data-oriented end, look up ECS overviews (and, if you ever need it at scale, a dedicated ECS library for Godot) once you have a reason to scale to thousands of entities.',
    quiz: [
      ...
      { q: 'How does this map onto a real engine?', a: 'In Godot you assemble a **scene** from a tree of **Nodes**, each node a focused capability you give behaviour with an attached script ([[e2-02]]). Different objects are just different node mixes — the exact composition idea from this lesson (and the same idea elsewhere as Unity components or Unreal Actor components), which is why it feels natural once you reach the engine layer.' }
    ],
