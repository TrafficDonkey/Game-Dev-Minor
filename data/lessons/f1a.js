...
'Scope check, again: a slice space is *rooms, not a region*. One entrance, one moment, one exit. If you are building a second area, you have left the slice.',
    steps: [
      ...
      'Export it correctly: FBX or glTF, correct scale and units, sane pivot, clear name ([[d5-03]], [[d5-04]]). Import and confirm it lands at the right size with no scale surprises.',
...
'Block out and partly dress the slice space. Deliverable: (1) a paper or tool sketch of the flow (entrance, mechanic moment, beat, exit); (2) a greybox of the space at correct scale that you have "walked" against your mechanic; (3) **one** game-ready asset you modelled and exported yourself, placed to do real work and reused modularly. Write three lines on what the *grey* taught you that you would have missed if you had textured first.',
    ...
    tags: ['blockout', 'greybox', 'modular', 'game-ready asset', 'level design'] },
  {
    ...
    concept:
'*Engine-specific (Unity/C#) — the same idea is a `Tick()` `UFUNCTION`/test in Unreal, and `_process()` plus GUT/GoDoTest in Godot. The transferable concept underneath is the same: the mechanic is logic that runs in the loop, reads input, respects delta time, and can be tested in isolation.*\n\n' +
...
'Separate the mechanic into two halves you already know. The **logic** is plain, engine-neutral state and rules ([[e1-07]]): given input and the world, what should happen? Keep this in a small, testable piece. The **engine glue** wires that logic into the loop, in Unity a `MonoBehaviour` reading input in `Update()` and moving things with `Time.deltaTime` ([[e1-02]]), physics-sensitive work in `FixedUpdate()`. The split is the whole point of the two-layer pillar: the rules are transferable, only the glue is engine-specific ([[e2-01]]).\n\n' +
...
'Then **test the mechanic** ([[e2-08]]). You cannot run an engine in this site, but the *logic* half is pure and testable anywhere, which is exactly why you split it out. A test pins behaviour: "from grounded, a jump press sets upward velocity"; "you cannot double-jump in the air"; "after `dt`, position advanced by `speed * dt`". When integration ([[f1-05]]) starts breaking things, your tests tell you instantly whether the *mechanic* still holds.',
    steps: [
      ...
      'In Unity, write the glue: a `MonoBehaviour` that reads the just-pressed event in `Update()`, calls the logic, and applies the result (move per `Time.deltaTime`; put physics in `FixedUpdate()`).',
      ...
      'Write tests against the *pure logic* (in Unity, an EditMode test with the Test Runner; the same asserts work on the JS core in the playground):',
...
'Code your one mechanic with the logic/glue split. Deliverable: (1) the **pure logic** as pseudocode or a JS function (prototype it in the **Code playground** above); (2) a short description of the **engine glue** (which Unity callback reads input, where delta time and FixedUpdate apply); (3) at least **two tests** of the pure logic stating exact expected behaviour (e.g. "no double-jump", "speed scales with dt"); (4) two feel additions you would add and why. If you have the engine, build it for real; if not, the JS core plus the glue description is a complete deliverable.',
    ...
    simplified: 'Test-framework specifics differ (Unity Test Runner / NUnit, Godot GUT, custom asserts); the JS asserts here illustrate the *idea* of pinning behaviour. The durable habit is keeping the mechanic’s logic pure enough to test at all.',
    goDeeper: 'Robert Nystrom’s *Game Programming Patterns* (free online) for the loop/state side; your engine’s testing docs (Unity Test Runner, Godot GUT) for the test side; and any "game feel" talk for the juice that makes one verb satisfying.',
    quiz: [
      { q: 'Why split the mechanic into pure "logic" and engine "glue"?', a: 'Because the logic (rules and state) is engine-neutral and transferable, while only the glue (which callback fires, how input and rendering attach) is engine-specific. The split keeps the rules portable across Unity/Unreal/Godot and, crucially, lets you unit-test the pure logic in isolation without running the engine.' },
    ...
    tags: ['mechanic', 'unit test', 'delta time', 'unity', 'logic vs glue'] },
  {
