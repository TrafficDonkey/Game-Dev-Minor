...
;(function (root) {
  ...
  D.lessons.push(
  {
    ...
    concept:
'*Layer-1, engine-neutral: vectors are the same maths in Unity, Unreal and Godot — only the type names change. We point to those names so you see what transfers.*\n\n' +
...
'Transforms also nest: a turret parented to a tank has a **local** transform (relative to the tank) and a **world** transform (relative to the scene). Move the tank and the turret follows for free. Names you will meet for the vector type: Unity `Vector3`, Unreal `FVector`, Godot `Vector3`. Same three numbers everywhere.',
    task:
...
'You rarely write the formulas: Unity gives `Vector3.Dot`, `Vector3.Cross`, `Vector3.Lerp`/`Mathf.Lerp`; Godot and Unreal mirror them. Knowing *which tool answers which question* is the actual skill — and it transfers to whatever engine the minor uses.',
    task:
'In the **Code playground**: (1) write `dot(a, b)` and test it on `(1,0)`·`(1,0)` (expect 1), `(1,0)`·`(0,1)` (expect 0) and `(1,0)`·`(-1,0)` (expect -1). (2) Write `lerp(a, b, t)` and print `lerp(0, 100, 0.25)` (expect 25). (3) On paper, pick the right tool for each: "is the player within a 60° view cone?", "which way should the guard turn to face the noise?", "fade the screen from black to clear over 1 second". Name dot, cross or lerp for each and one sentence why.',
    ...
    tags: ['dot product', 'cross product', 'lerp', 'normalize', 'vector math', 'engine-neutral'] },
  {
    ...
    concept:
'*Layer-1, engine-neutral. Engines bundle a physics engine (Unity uses PhysX, Godot its own, Unreal Chaos) — you mostly *configure* it, but understanding the model is what makes it stop fighting you.*\n\n' +
...
'**Static vs dynamic, and tunnelling.** Mark never-moving geometry **static** so the engine can optimise it; moving bodies are **dynamic** (or **kinematic** — moved by code, not forces). One bug to know by name: **tunnelling**, where a fast object skips *through* a thin wall because between two frames it teleported past it. The fix is **continuous collision detection** (sweep the shape along its path) — turn it on for bullets and fast players.\n\n' +
'**Triggers vs solid collisions.** A *solid* collision pushes objects apart and reports contact; a *trigger* overlaps freely and just fires an event (entered/exited a zone) — perfect for pickups, checkpoints and damage volumes, and the usual seam to gameplay and level scripting.\n\n' +
